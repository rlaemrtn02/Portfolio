...
{
	document.frm1.action = "boardBmodify.jsp";
	document.frm1.submit();
}

function submit_reply()
{
	document.frm1.action = "boardBreply.jsp";
	document.frm1.submit();
}

function submit_delete()
{
	document.frm1.action = "boardBdelConfirm.jsp";
	document.frm1.submit();
}

function submit_list()
{
	document.frm1.action = "boardBlist.jsp";
	document.frm1.submit();
}
</script>
...
try
{
	...
	String strSQL = "SELECT num, title, contents, writer, CONVERT(CHAR(10), updatedtm, 120) writedt, readcnt FROM boardB WHERE num = ?";
	...
	else
	{
		...
		strSQL = "UPDATE boardB SET readcnt = readcnt + 1 WHERE num = ?"; // 조회수를 증가
		pstmt = con.prepareStatement(strSQL);
		pstmt.setInt(1, num);
		pstmt.executeUpdate();
